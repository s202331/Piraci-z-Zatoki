...
szczegółowych informacji na swój temat, takich jak: status cywilny,
płeć, poziom dochodów, liczba dzieci, poziom wykształcenia, wykonywany
zawód, status posiadania domu, liczba samochodów, odległość do miejsca
pracy, region zamieszkania oraz wiek.
...
znajduje się 31 brakujących wartości. W zbiorze brakuje 0,2% wartości, w
kolumnach: Income, Children, Cars, Age. Sprawdzono również rozłożenie braków w 
kolumnach w wartościach procentowych.
...
4.  **Korelacje między zmiennymi** - Macierz korelacji wykazuje, że
    najwyższa zależność występuje między liczbą dzieci a dochodem (0,53)
...
Hipotezy testowe:
H₀ (hipoteza zerowa): Dane mają rozkład normalny.
H₁ (hipoteza alternatywna): Dane nie mają rozkładu normalnego.
...
## Wizualizacja testóww 
```{r}
install.packages("ggstatsplot")  
library(ggstatsplot)

ggbetweenstats( data = sklep_rowerowy, x = Purchased.Bike, y = Region,
type = "nonparametric",
title = "Wiek a decyzja o zakupie roweru",
xlab = "Zakup roweru", ylab = "Wiek", )
...
Z przeprowadzonej analizy wynika iż głównymi powodami zakupu rowerów
jest (Commute.Distance) trasa do pokonania do pracy, oraz wiek i region.

Z przeprowadzonych testów Kruskala-Wallisa znaleziono zależność miedzy
trasą do pokonania a zakupem roweru, wykazały istotnie statystyczne
różnice w dystansie do pracy miedzy osobami ktore kupiły rower a osobami
które go nie kupiły. Oznacza to że dystans do pracy jest kluczowym
czynnikiem wpływającym na zakup roweru. Im mniejsza odległość do pracy
tym chętniej kupujemy rower.
...
library(rmarkdown)
library(knitr)
rmarkdown::render("Rmarkdown projekt.Rmd", output_format = "html_document")
```
