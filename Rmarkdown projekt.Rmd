...
Przeprowadzona analiza miała na celu określenie kluczowych czynników wpływających na decyzję klientów o zakupie roweru, uwzględniając zarówno aspekty demograficzne, ekonomiczne, jak i behawioralne. Proces badawczy obejmował kompleksowe przygotowanie danych, ich wizualizację, analizę opisową oraz wnioskowanie statystyczne, co pozwoliło na uzyskanie pełnego obrazu struktury klientów i ich preferencji zakupowych.

Na początkowym etapie analizy dokonano czyszczenia i transformacji danych, w tym imputacji brakujących wartości oraz konwersji zmiennych na odpowiednie formaty, co umożliwiło poprawne przeprowadzenie dalszych badań. Standaryzacja zmiennych numerycznych oraz przypisanie wartości kategorycznych do odpowiednich grup pozwoliły na precyzyjniejsze analizy statystyczne i predykcyjne. Przeprowadzono także identyfikację i wizualizację rozkładu zmiennych, co pomogło w ocenie jakości danych i wykryciu potencjalnych anomalii.

Wizualizacja danych umożliwiła lepsze zrozumienie struktury populacji klientów sklepu rowerowego oraz identyfikację potencjalnych zależności. Wykresy pudełkowe i histogramy pozwoliły na ocenę rozkładu dochodów, wieku, liczby dzieci oraz samochodów w badanej grupie. Wykresy słupkowe zilustrowały strukturę populacji pod względem płci, poziomu wykształcenia, regionu zamieszkania oraz decyzji o zakupie roweru. Z kolei macierz korelacji pozwoliła na identyfikację zmiennych, które mogą mieć istotny wpływ na analizowane zależności. 

Analiza opisowa dostarczyła istotnych informacji dotyczących cech klientów. Średni dochód klientów wynosił 56 252,1, jednak duża zmienność wartości dochodów wskazywała na wyraźne różnice w statusie ekonomicznym badanych osób. Wiek klientów obejmował szeroki zakres od 25 do 89 lat, przy średniej 44,2, co sugeruje, że sklep rowerowy dociera do klientów o zróżnicowanych profilach wiekowych. Struktura demograficzna pokazała, że kobiety i mężczyźni są reprezentowani w niemal równych proporcjach, a ponad połowa klientów pozostaje w związku małżeńskim. Większość badanej populacji posiada wykształcenie wyższe, a dominującą grupę zawodową stanowią osoby wykonujące prace profesjonalne. Ponadto, ponad 68% klientów posiada własną nieruchomość, co może wskazywać na stabilną sytuację finansową.

Jednym z kluczowych ustaleń było to, że dystans do pracy okazał się jednym z najważniejszych czynników determinujących zakup roweru. Osoby mieszkające bliżej miejsca pracy (0-1 mil) znacznie częściej decydowały się na jego zakup w porównaniu do tych, które muszą pokonywać dłuższe dystanse. Testy statystyczne wykazały istotne różnice w decyzjach zakupowych w zależności od dystansu do pracy, co potwierdziły zarówno test Kruskala-Wallisa, jak i analiza regresyjna. Wiek również miał pewien wpływ na decyzję o zakupie roweru – młodsze osoby częściej dokonywały zakupu, choć różnice te nie były dominujące. Dochód, mimo początkowych przypuszczeń, nie okazał się czynnikiem istotnie wpływającym na decyzję zakupową, co potwierdziły zarówno test Manna-Whitneya, jak i regresja logistyczna. Mediana dochodu była taka sama dla obu grup, co sugeruje, że osoby o różnych poziomach zarobków podejmują decyzję o zakupie roweru w podobny sposób.

Model regresji logistycznej dostarczył bardziej precyzyjnych informacji dotyczących czynników determinujących zakup roweru. Wyniki analizy wykazały, że osoby samotne były bardziej skłonne do zakupu roweru niż osoby pozostające w związkach małżeńskich, co może wynikać z większej mobilności i elastyczności transportowej. Ponadto, liczba dzieci oraz liczba posiadanych samochodów miały negatywny wpływ na decyzję o zakupie roweru – każda dodatkowa osoba na utrzymaniu oraz dodatkowy samochód zmniejszały prawdopodobieństwo zakupu. Natomiast mieszkańcy regionu Pacyfiku częściej decydowali się na zakup roweru, co może być związane z infrastrukturą rowerową oraz preferencjami ekologicznymi tego regionu.

Testy statystyczne potwierdziły również istotną zależność między zakupem roweru a regionem zamieszkania. Osoby z niektórych regionów były bardziej skłonne do zakupu roweru niż inne, co może wynikać z uwarunkowań środowiskowych, dostępności infrastruktury rowerowej czy polityki transportowej w danym obszarze. Z kolei testy chi-kwadrat wykazały istotny wpływ dystansu do pracy na decyzję zakupową, potwierdzając, że osoby dojeżdżające na krótsze odległości są bardziej skłonne do korzystania z roweru jako środka transportu.

Podsumowując, przeprowadzona analiza pozwoliła na wyciągnięcie kilku kluczowych wniosków. Dystans do pracy okazał się najważniejszym czynnikiem determinującym decyzję o zakupie roweru – osoby mieszkające bliżej miejsca pracy znacznie częściej wybierają ten środek transportu. Wiek miał umiarkowany wpływ na zakup roweru, jednak dochód nie był istotnym czynnikiem decydującym. Liczba posiadanych samochodów oraz dzieci zmniejszała prawdopodobieństwo zakupu roweru, co sugeruje, że osoby o bardziej ustabilizowanej sytuacji rodzinnej częściej wybierają inne środki transportu. Różnice regionalne miały pewien wpływ na decyzję o zakupie, zwłaszcza w przypadku regionu Pacyfiku, gdzie odnotowano większą skłonność do korzystania z rowerów.

Wnioski płynące z analizy mogą być wykorzystane w strategii marketingowej sklepu rowerowego. Warto skoncentrować się na promocji rowerów wśród osób mieszkających blisko miejsca pracy, ponieważ są one bardziej skłonne do ich zakupu. Kampanie mogą być skierowane szczególnie do klientów młodszych, ponieważ to właśnie oni częściej decydują się na rower jako środek transportu lub narzędzie rekreacyjne. Wyniki analizy sugerują także, że region zamieszkania może wpływać na decyzję zakupową, dlatego dostosowanie działań marketingowych do specyfiki poszczególnych regionów może zwiększyć sprzedaż. Ponadto, uwzględnienie alternatywnych rozwiązań transportowych dla klientów dojeżdżających na większe odległości, takich jak rowery elektryczne, może poszerzyć bazę potencjalnych nabywców.

Przeprowadzona analiza dostarczyła cennych informacji, które mogą wspomóc strategiczne decyzje operacyjne oraz marketingowe sklepu rowerowego. Pozwoliła także na określenie najważniejszych czynników wpływających na decyzję zakupową klientów, co umożliwia lepsze dopasowanie oferty do ich preferencji oraz optymalizację działań sprzedażowych i promocyjnych.