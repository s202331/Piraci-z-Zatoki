...
Przekrój na podstawie decyzji o zakupie roweru a średnim dochodem i
wiekiem.Przekrój na podstawie decyzji o zakupie roweru a średnim
dochodem i wiekiem.Przekrój na podstawie dystansu do pracy a regionem a
średnim dochodem
...
Marital.StatusSingle (p = 0.00904): Bycie singlem zwiększa
prawdopodobieństwo zakupu roweru (wartość współczynnika = 0.4755,
istotność na poziomie 0.01).

Children (p = 0.03568): Każde dodatkowe dziecko zmniejsza szansę zakupu
roweru (wartość współczynnika = -0.1342, istotność na poziomie 0.05).

EducationPartial College (p = 0.02652): Osoby z częściowym
wykształceniem wyższym mają mniejszą szansę zakupu roweru (współczynnik
= -0.5856).

Home.OwnerYes (p = 0.01264): Posiadanie domu zwiększa prawdopodobieństwo
zakupu roweru (współczynnik = 0.5083, istotność na poziomie 0.05).

Cars (p = 0.00555): Każdy dodatkowy samochód zmniejsza szansę zakupu
roweru (współczynnik = -0.3000, istotność na poziomie 0.01).

Commute.Distance10+ Miles (p = 0.00197): Osoby dojeżdżające ponad 10 mil
mają znacznie mniejsze szanse na zakup roweru (współczynnik = -1.091,
istotność na poziomie 0.01).

RegionPacific (p = 0.00999): Osoby mieszkające w regionie Pacyfiku
częściej kupują rower (współczynnik = 0.7637). Niektóre zmienne nie mają
istotnego wpływu na decyzję o zakupie roweru:

GenderMale (p = 0.86498) – płeć męska nie ma znaczącego wpływu na zakup
roweru. Income (p = 0.07602) – dochód jest na granicy istotności, ale
jego wpływ jest bardzo mały. EducationGraduate Degree, EducationHigh
School – poziom wykształcenia nie ma dużego wpływu. Occupation – rodzaj
zawodu nie ma znaczącego wpływu. Commute.Distance1-2 Miles, 2-5 Miles,
5-10 Miles – krótsze dystanse do pracy nie wykazują istotnego wpływu.
...
Wnioski Z przeprowadzonej analizy wynika iż głównymi powodami zakupu
rowerów jest (Commute.Distance) trasa do pokonania do pracy, oraz wiek i
region.

W ramach analizy statystycznej przeprowadzono również segmentację
klientów na podstawie profilu klientów, co pozwoliło zidentyfikować
różne grupy konsumentów.

Podsumowując, wyniki analizy wskazują na dużą koncentrację klientów w
określonych grupach wiekowych oraz o dużej zależności miedzy odległością
do pracy a zakupem roweru. Przeprowadzone analizy pozwalają na lepsze
zrozumienie potrzeb klientów, co może przyczynić się do poprawy
efektywności działań marketingowych i sprzedażowych sklepu rowerowego.
Data Wrangling
...
Wnioskowanie statystyczne Z przeprowadzonych testów Kruskala-Wallisa
znaleziono zależność miedzy trasą do pokonania a zakupem roweru,
wykazały istotnie statystyczne różnice w dystansie do pracy miedzy
osobami ktore kupiły rower a osobami które go nie kupiły. Oznacza to że
dystans do pracy jest kluczowym czynnikiem wpływającym na zakup roweru.
